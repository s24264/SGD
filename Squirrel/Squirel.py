...
def checkLevelUp(score, level, levelUpTime):
    global FPS
    newLevel = level
    newLevelUpTime = levelUpTime

    if score % 5 == 0 and score != 0 and time.time() - levelUpTime > 7:
        newLevel += 1
        FPS = min(FPS + 2, 60)
        newLevelUpTime = time.time()

    if time.time() - newLevelUpTime < 7:
        levelUpSurf = BASICFONT.render('Level Up!', True, WHITE)
        levelUpRect = levelUpSurf.get_rect()
        levelUpRect.center = (HALF_WINWIDTH, 50)
        DISPLAYSURF.blit(levelUpSurf, levelUpRect)

    return newLevel, newLevelUpTime
            ...
            for i in range(len(squirrelObjs)-1, -1, -1):
                ...
                if 'rect' in sqObj and playerObj['rect'].colliderect(sqObj['rect']):
                    ...
                    if sqObj['width'] * sqObj['height'] <= playerObj['size']**2:
                        ...
                        del squirrelObjs[i]
                        score += 1
                        level, levelUpTime = checkLevelUp(score, level, levelUpTime)
